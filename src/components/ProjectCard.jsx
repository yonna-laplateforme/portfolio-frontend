...
const mediaSrc = (url) =>
  isVideoUrl(url) || url?.includes('.gif') ? url : getOptimizedUrl(url, 1200);

const ProjectCard = ({ project, index }) => {
  const navigate = useNavigate();
  const cardRef = useRef(null);

  // On sépare les médias
          ...
          <div className="flex gap-2 mt-4">
            {thumbnails.map((thumb, i) => (
              <div key={i} className="w-16 h-12 overflow-hidden border border-zinc-800">
                {isVideoUrl(thumb) ? (
                  <video src={thumb} muted autoPlay loop playsInline className="w-full h-full object-cover opacity-60 hover:opacity-100 transition-opacity" />
                ) : (
                  <img
                    src={mediaSrc(thumb)}
                    alt={`Miniature ${i + 1}`}
